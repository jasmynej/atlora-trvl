...
```ts
platformProcedure  // platform subject — destination catalog, platform config
agencyProcedure    // agency subject; agency_id from session — trips, clients, CRM, agency profile
travelerProcedure  // traveler subject; traveler_id from session — profile vault, saved trips, self-planned itineraries, trip hub
```
...
- **Auth:** Session cookie, agency subject (`agency_admin`, `agency_staff`)
- **Design theme:** `admin`
...
- **Auth:** Session cookie, traveler subject
...
- **Auth:** Session token in Expo SecureStore, sent as a bearer header — cookies are not idiomatic in React Native
...
- Schema is defined in TypeScript under `packages/db`; migrations are generated with `drizzle-kit` and committed as SQL
- Because migrations are plain SQL, **Row-Level Security policies, pgvector index definitions, and any other Postgres feature Drizzle does not model are written by hand in migration files** and are part of the reviewed schema, not out-of-band setup
- Types are inferred from the schema rather than generated by a separate codegen step, which keeps `packages/types` the home for Zod contracts and `packages/db` the home for row shapes
...
> **Status:** First-party and local-first, across all three surfaces. Chosen for development simplicity — nothing external to configure, no organization model to work around, and full control over the session shape. **Not production-grade.** See *Migration path* below.

- **Credentials:** email + password, hashed with `argon2id` via a maintained library. No hand-rolled crypto.
- **Sessions:** an opaque random token stored in a `sessions` row and delivered in an `httpOnly`, `SameSite=Lax`, `Secure` cookie. Server-side sessions rather than self-contained tokens, because revocation matters more than statelessness at this scale — suspending an agency or a platform admin has to end their session immediately, and a stateless token cannot do that without a blocklist that reintroduces the database read anyway.
- **Single domain** means one cookie covers every web surface with no cross-origin handling.
- **Mobile** cannot use cookies idiomatically. `apps/mobile` stores the same opaque token in Expo SecureStore and sends it as a bearer header; server-side resolution is identical.
- **Roles:** `platform_admin`, `platform_editor`, `agency_admin`, `agency_staff`, `traveler`.

### Subject types

A session resolves to exactly one subject, matching the data ownership model:

| Subject | Session carries | Guard |
|---|---|---|
| Platform | `platform_user_id`, role | `platformProcedure` |
| Agency | `agency_user_id`, `agency_id`, role | `agencyProcedure` |
| Traveler | `traveler_id` | `travelerProcedure` |

The session record carries an explicit `subject_type` discriminant, and every procedure asserts on it directly. **Subject is never inferred from the presence or absence of a field.** This replaces the org-claim check the previous provider-based model relied on, and it is a stronger invariant: absence of an organization was ambiguous and could be produced by misconfiguration, whereas a discriminant has to be set to something wrong deliberately.

A single human may hold both a traveler identity and an agency staff identity. These remain distinct subjects and must not be merged — switching between them is a new session, not a mode toggle.

> **Open decision — identity table shape.** Whether the three subject types share one credential row (a single `users` table, with membership rows pointing at platform, agency, or traveler profiles) or hold three fully separate identity tables is **unresolved**. Shared credentials avoid making an advisor re-register to use the platform as a traveler; separate tables make the "distinct subjects" rule structural rather than conventional. Neither is settled — do not build as though it were.

### Migration path

All auth logic lives behind one module exposing a narrow interface — `authenticate`, `createSession`, `resolveSession`, `revokeSession`. Procedures depend on that interface and never on its storage. Moving to a managed provider later should touch that module and the sign-in screens, and nothing else.

### Deliberately not implemented

MFA, email verification, password reset, device management, breach detection, SSO. Login rate limiting and account lockout **are** implemented — they are cheap, and their absence is exploitable even in a small deployment.

> **Real traveler PII must not be stored under this auth system.** The Epic D profile vault holds passport and ID documents. Until auth is production-grade, seed and synthetic data only.
...
| Auth | First-party — argon2id + server-side sessions (pre-production) |
...
**Why first-party auth instead of a managed provider?**
The tradeoff currently favors control and local simplicity: nothing external to configure, no organization model to bend around a platform admin who has no organization, and a session shape that maps onto the three-subject ownership model directly rather than being approximated by claims. The cost is real — no MFA, no breach monitoring, no independently audited implementation — which is why it is scoped as pre-production and confined behind a single module. The trigger to revisit is not growth; it is the moment real traveler documents enter the system.
