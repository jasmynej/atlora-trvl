...
export function PlatformAuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // error UI on 401.
  const meQuery = platformTrpc.platform.auth.me.useQuery(undefined, { retry: false })

  const loginMutation = platformTrpc.platform.auth.login.useMutation({
    onSuccess: (user) => utils.platform.auth.me.setData(undefined, user),
  })
  const logoutMutation = platformTrpc.platform.auth.logout.useMutation({
